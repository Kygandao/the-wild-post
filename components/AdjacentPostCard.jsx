...
const AdjacentPostCard = ({ post, position }) => (
  <>
    ...
    <div className='absolute flex flex-col items-center justify-center w-full h-full p-4 rounded-lg'>
      <p className='text-xs font-semibold text-white text-shadow'>{moment(post.createdAt).format('MMM DD, YYYY')}</p>
      <p className='text-2xl font-semibold text-center text-white text-shadow'>{post.title}</p>
    </div>
    <Link href={`/post/${post.slug}`}><span className='absolute z-10 w-full h-full cursor-pointer' /></Link>
    {position === 'LEFT' && (
      <div className='absolute py-2 text-center bg-green-600 rounded-full cursor-pointer arrow-btn bottom-5 left-4'>
    ...
    {position === 'RIGHT' && (
      <div className='absolute py-2 text-center bg-green-600 rounded-full cursor-pointer arrow-btn bottom-5 right-4'>
        <svg xmlns='http://www.w3.org/2000/svg' className='w-6 w-full h-6 text-white' fill='none' viewBox='0 0 24 24' stroke='currentColor'>
